...
export default createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(update.fulfilled, (state, action) => {
      ...
      state.department = action.payload.data;
      const departmentIndex = state.departments.findIndex((p) => p.id === action.payload.data.id)
      state.departments = state.departments.filter((p) => p.id !== action.payload.data.id)
      state.departments.splice(departmentIndex, 0, action.payload.data)
