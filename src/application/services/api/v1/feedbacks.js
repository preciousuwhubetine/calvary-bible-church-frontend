...
export default createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(update.fulfilled, (state, action) => {
      ...
      state.feedback = action.payload.data;
      const feedbackIndex = state.feedbacks.findIndex((p) => p.id === action.payload.data.id)
      state.feedbacks = state.feedbacks.filter((p) => p.id !== action.payload.data.id)
      state.feedbacks.splice(feedbackIndex, 0, action.payload.data)
