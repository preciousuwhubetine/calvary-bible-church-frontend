...
const entity = 'store_items'

const endpoints = {
  index: (store_item_category_id, page, per_page) => `/${entity}?store_item_category_id=${store_item_category_id}&page=${page}&per_page=${per_page}`,
  create: entity,
  show: (id) => `/${entity}/${id}`,
  update: (id) => `/${entity}/${id}`,
...
export const index = createAsyncThunk(
  ...
  async (data, thunkAPI) => {
    try {
      const response = await api.get(endpoints.index(data.store_item_category_id, data.page, data.per_page));
...
export default createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(index.fulfilled, (state, action) => {
      state.loading = false;
      state.store_items = action.payload.data;
      state.error = null;
    })
    ...
    builder.addCase(create.fulfilled, (state, action) => {
      ...
      state.store_items.push(action.payload.data);
    ...
    builder.addCase(show.fulfilled, (state, action) => {
      ...
      state.store_item = action.payload.data;
    ...
    builder.addCase(update.fulfilled, (state, action) => {
      ...
      state.store_item = action.payload.data;
      const store_itemIndex = state.store_items.findIndex((p) => p.id === action.payload.data.id)
      state.store_items = state.store_items.filter((p) => p.id !== action.payload.data.id)
      state.store_items.splice(store_itemIndex, 0, action.payload.data)
