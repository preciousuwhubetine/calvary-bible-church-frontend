...
export default createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(update.fulfilled, (state, action) => {
      ...
      state.event = action.payload.data;
      const eventIndex = state.events.findIndex((p) => p.id === action.payload.data.id)
      state.events = state.events.filter((p) => p.id !== action.payload.data.id)
      state.events.splice(eventIndex, 0, action.payload.data)
