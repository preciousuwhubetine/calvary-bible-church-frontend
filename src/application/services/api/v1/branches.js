...
export default createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(update.fulfilled, (state, action) => {
      ...
      state.branch = action.payload.data;
      const branchIndex = state.branches.findIndex((p) => p.id === action.payload.data.id)
      state.branches = state.branches.filter((p) => p.id !== action.payload.data.id)
      state.branches.splice(branchIndex, 0, action.payload.data)
